...
public final class BenchRunner {
	...
	public static void main(String[] args) {
		String configFile = "ressources/config/config_without_overlap.txt";
		...
		for (int size : sizes) {
			...
			for (Mode mode : modes) {
                ...
                long compressedBits = ((long) bp.getWords().length) * 32L; // adapte si ton API diffère
                double bitsPerInt = compressedBits / (double) size;
                double ratio = bitsPerInt / 32.0;
                
                // Log console court
                System.out.printf(Locale.US,
                        "[OK] mode=%s, data=%s, n=%d | comp=%.3f ns/int, decomp=%.3f ns/int, get=%.3f ns, ratio=%.3f%n",
                        mode.name(), configFile, size, tCompNsPerInt, tDecompNsPerInt, tGetNs, ratio);
